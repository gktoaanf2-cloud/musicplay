...
(function() {
    ...
    function saveToSupabase() {
        ...
        dbSaveTimer = setTimeout(async function() {
            try {
                ...
                await _supabase.from('playlists').update({pair_name:d.pairName,sub_title:d.subTitle,font_size:d.fontSize,is_bodoni:d.isBodoni,color_1:d.color1,color_2:d.color2,color_3:d.color3,color_4:d.color4,cover_image_url:d.imageData||null}).eq('id',PL_ID);
    ...
    window.addEventListener('load', async function() {
        ...
        // 플레이어바 반투명 배경 (밝은 테마에서 가시성 확보)
        var pbar=document.querySelector('.player-bar');
        if(pbar)pbar.style.cssText+='background:rgba(3,3,5,0.55);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);padding:14px 20px;margin-left:-20px;margin-right:-20px;border-top:1px solid rgba(255,255,255,0.04);';

        // ===================== ARCH 패널 (갤러리형) =====================
        var archPanel=document.createElement('div');archPanel.id='arch-panel';
        archPanel.style.cssText='display:none;position:fixed;inset:0;z-index:100000;background:rgba(0,0,0,0.8);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);justify-content:center;align-items:center;opacity:0;transition:0.3s;overflow-y:auto;padding:40px 20px;';
        archPanel.innerHTML='<div style="width:680px;max-width:96vw;position:relative;">'+
