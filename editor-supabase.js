...
(function() {
    ...
    window.addEventListener('load', async function() {
        ...
        // --- 유튜브 썸네일 자동 대체 (개별 이미지 없을 때) ---
        var useYtThumb=true;
        var _origPlayTrack=window.playTrack;
        window.playTrack=function(idx){
            _origPlayTrack(idx);
            if(useYtThumb && idx>=0 && idx<window.playlistData.length){
        ...
        if(tools){
            ...
            ytBtn.style.cssText='background:transparent;border:none;color:rgba(255,255,255,0.35);font-size:9px;cursor:pointer;transition:0.3s;font-family:inherit;letter-spacing:1.5px;font-weight:600;text-transform:uppercase;';
            ytBtn.textContent='YT THUMB: ON';ytBtn.title='유튜브 썸네일 자동 표시';
            ytBtn.onclick=function(){useYtThumb=!useYtThumb;ytBtn.textContent='YT THUMB: '+(useYtThumb?'ON':'OFF');ytBtn.style.color=useYtThumb?'rgba(255,255,255,0.35)':'rgba(255,255,255,0.12)';};
            ytBtn.onmouseover=function(){this.style.color='var(--color-1)';};
            ytBtn.onmouseout=function(){this.style.color=useYtThumb?'rgba(255,255,255,0.35)':'rgba(255,255,255,0.12)';};
            delRow.appendChild(ytBtn);
